...
var GlyphRenderer_exports = {};
__export(GlyphRenderer_exports, {
  GlyphRenderer: () => GlyphRenderer
});
module.exports = __toCommonJS(GlyphRenderer_exports);
...
var VertexAttribLocations = /* @__PURE__ */ ((VertexAttribLocations2) => {
  VertexAttribLocations2[VertexAttribLocations2["UNIT_QUAD"] = 0] = "UNIT_QUAD";
  VertexAttribLocations2[VertexAttribLocations2["CELL_POSITION"] = 1] = "CELL_POSITION";
  VertexAttribLocations2[VertexAttribLocations2["OFFSET"] = 2] = "OFFSET";
  VertexAttribLocations2[VertexAttribLocations2["SIZE"] = 3] = "SIZE";
  VertexAttribLocations2[VertexAttribLocations2["TEXPAGE"] = 4] = "TEXPAGE";
  VertexAttribLocations2[VertexAttribLocations2["TEXCOORD"] = 5] = "TEXCOORD";
  VertexAttribLocations2[VertexAttribLocations2["TEXSIZE"] = 6] = "TEXSIZE";
...
let $clippedPixels = 0;
class GlyphRenderer extends import_lifecycle.Disposable {
  constructor(_terminal, _gl, _dimensions, _optionsService) {
    ...
    const gl = this._gl;
    if (import_TextureAtlas.TextureAtlas.maxAtlasPages === void 0) {
      import_TextureAtlas.TextureAtlas.maxAtlasPages = Math.min(32, (0, import_RendererUtils.throwIfFalsy)(gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS)));
      import_TextureAtlas.TextureAtlas.maxTextureSize = (0, import_RendererUtils.throwIfFalsy)(gl.getParameter(gl.MAX_TEXTURE_SIZE));
    }
    this._program = (0, import_RendererUtils.throwIfFalsy)((0, import_WebglUtils.createProgram)(gl, vertexShaderSource, createFragmentShaderSource(import_TextureAtlas.TextureAtlas.maxAtlasPages)));
    ...
    this._resolutionLocation = (0, import_RendererUtils.throwIfFalsy)(gl.getUniformLocation(this._program, "u_resolution"));
    this._textureLocation = (0, import_RendererUtils.throwIfFalsy)(gl.getUniformLocation(this._program, "u_texture"));
    ...
    const textureUnits = new Int32Array(import_TextureAtlas.TextureAtlas.maxAtlasPages);
    for (let i = 0; i < import_TextureAtlas.TextureAtlas.maxAtlasPages; i++) {
      textureUnits[i] = i;
    }
    ...
    this._atlasTextures = [];
    for (let i = 0; i < import_TextureAtlas.TextureAtlas.maxAtlasPages; i++) {
      const glTexture = new import_WebglUtils.GLTexture((0, import_RendererUtils.throwIfFalsy)(gl.createTexture()));
      this._register((0, import_lifecycle.toDisposable)(() => gl.deleteTexture(glTexture.texture)));
  ...
  _updateCell(array, x, y, code, bg, fg, ext, chars, width, lastBg) {
    ...
    if (code === import_Constants.NULL_CELL_CODE || code === void 0) {
    ...
    if (this._optionsService.rawOptions.rescaleOverlappingGlyphs) {
      if ((0, import_RendererUtils.allowRescaling)(code, width, $glyph.size.x, this._dimensions.device.cell.width)) {
