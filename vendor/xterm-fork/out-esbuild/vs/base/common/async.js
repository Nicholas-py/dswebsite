...
__export(async_exports, {
  ...
  AsyncIterableSource: () => AsyncIterableSource,
  AutoOpenBarrier: () => AutoOpenBarrier,
  Barrier: () => Barrier,
  ...
  LazyStatefulPromise: () => LazyStatefulPromise,
  LimitedQueue: () => LimitedQueue,
  Limiter: () => Limiter,
  ProcessTimeRunOnceScheduler: () => ProcessTimeRunOnceScheduler,
  Promises: () => Promises,
  Queue: () => Queue,
  RunOnceScheduler: () => RunOnceScheduler,
  RunOnceWorker: () => RunOnceWorker,
  Sequencer: () => Sequencer,
  SequencerByKey: () => SequencerByKey,
  StatefulPromise: () => StatefulPromise,
  TaskSequentializer: () => TaskSequentializer,
  ThrottledDelayer: () => ThrottledDelayer,
  ThrottledWorker: () => ThrottledWorker,
  Throttler: () => Throttler,
...
var import_lazy = require("vs/base/common/lazy");
function isThenable(obj) {
  return !!obj && typeof obj.then === "function";
}
function createCancelablePromise(callback) {
  const source = new import_cancellation.CancellationTokenSource();
...
class Delayer {
  ...
  trigger(task, delay = this.defaultDelay) {
    ...
    this.deferred = delay === import_symbols.MicrotaskDelay ? microtaskDeferred(fn) : timeoutDeferred(delay, fn);
  ...
  cancel() {
    this.cancelTimeout();
    if (this.completionPromise) {
      this.doReject?.(new import_errors.CancellationError());
      this.completionPromise = null;
    }
  }
  cancelTimeout() {
...
class Limiter {
  constructor(maxDegreeOfParalellism) {
    ...
    this._onDrained = new import_event.Emitter();
  }
  /**
   *
   ...
   */
  whenIdle() {
    return this.size > 0 ? import_event.Event.toPromise(this.onDrained) : Promise.resolve();
  }
  get onDrained() {
...
class TimeoutTimer {
  ...
  cancelAndSet(runner, timeout2) {
    if (this._isDisposed) {
      throw new import_errors.BugIndicatingError(`Calling 'cancelAndSet' on a disposed TimeoutTimer`);
    }
    this.cancel();
  ...
  setIfNotSet(runner, timeout2) {
    if (this._isDisposed) {
      throw new import_errors.BugIndicatingError(`Calling 'setIfNotSet' on a disposed TimeoutTimer`);
    }
    if (this._token !== -1) {
      return;
    }
...
class IntervalTimer {
  ...
  cancelAndSet(runner, interval, context = globalThis) {
    if (this.isDisposed) {
      throw new import_errors.BugIndicatingError(`Calling 'cancelAndSet' on a disposed IntervalTimer`);
    }
    this.cancel();
    const handle = context.setInterval(() => {
      runner();
    }, interval);
    this.disposable = (0, import_lifecycle.toDisposable)(() => {
...
class ThrottledWorker extends import_lifecycle.Disposable {
  constructor(options, handler) {
    super();
    this.options = options;
    this.handler = handler;
    this.pendingWork = [];
    this.throttler = this._register(new import_lifecycle.MutableDisposable());
    this.disposed = false;
  }
  /**
...
class DeferredPromise {
  ...
  cancel() {
    return this.error(new import_errors.CancellationError());
  }
}
var Promises;
((Promises2) => {
...
class StatefulPromise {
  ...
  requireValue() {
    if (!this._isResolved) {
      throw new import_errors.BugIndicatingError("Promise is not resolved yet");
    }
    if (this._error) {
      throw this._error;
    }
...
class LazyStatefulPromise {
  constructor(_compute) {
    ...
    this._promise = new import_lazy.Lazy(() => new StatefulPromise(this._compute()));
  }
  /**
   * Returns the resolved value.
   * Throws if the promise is not resolved yet.
...
class AsyncIterableObject {
  ...
  constructor(executor, onReturn) {
    ...
    this._onStateChanged = new import_event.Emitter();
    ...
    return {
      next: async () => {
        do {
          ...
          await import_event.Event.toPromise(this._onStateChanged.event);
        } while (true);
      },
      return: async () => {
        this._onReturn?.();
...
function createCancelableAsyncIterable(callback) {
  ...
  return new CancelableAsyncIterableObject(source, async (emitter) => {
    const subscription = source.token.onCancellationRequested(() => {
      subscription.dispose();
      source.dispose();
      emitter.reject(new import_errors.CancellationError());
    });
    try {
