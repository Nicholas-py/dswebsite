...
var import_stopwatch = require("vs/base/common/stopwatch");
const _enableListenerGCedWarning = false;
const _enableDisposeWithListenerWarning = false;
const _enableSnapshotPotentialLeakWarning = false;
var Event;
((Event2) => {
  Event2.None = () => import_lifecycle.Disposable.None;
  ...
  function any(...events) {
    return (listener, thisArgs = null, disposables) => {
      const disposable = (0, import_lifecycle.combinedDisposable)(...events.map((event) => event((e) => listener.call(thisArgs, e))));
  ...
  function fromObservableLight(observable) {
    return (listener, thisArgs, disposables) => {
      ...
      if (disposables instanceof import_lifecycle.DisposableStore) {
...
class EventProfiling {
  ...
  start(listenerCount) {
    this._stopWatch = new import_stopwatch.StopWatch();
...
class Emitter {
  constructor(options) {
    ...
    this._leakageMon = _globalLeakWarningThreshold > 0 || this._options?.leakWarningThreshold ? new LeakageMonitor(options?.onListenerError ?? import_errors.onUnexpectedError, this._options?.leakWarningThreshold ?? _globalLeakWarningThreshold) : void 0;
  ...
  get event() {
    this._event ??= (callback, thisArgs, disposables) => {
      if (this._leakageMon && this._size > this._leakageMon.threshold ** 2) {
        ...
        const errorHandler = this._options?.onListenerError || import_errors.onUnexpectedError;
        errorHandler(error);
...
class AsyncEmitter extends Emitter {
  async fireAsync(data, token, promiseJoin) {
    ...
    if (!this._asyncDeliveryQueue) {
      this._asyncDeliveryQueue = new import_linkedList.LinkedList();
    }
      ...
      await Promise.allSettled(thenables).then((values) => {
        for (const value of values) {
          if (value.status === "rejected") {
            (0, import_errors.onUnexpectedError)(value.reason);
          }
        }
      });
    }
...
class PauseableEmitter extends Emitter {
  constructor(options) {
    super(options);
    this._isPaused = 0;
    this._eventQueue = new import_linkedList.LinkedList();
...
class EventMultiplexer {
  ...
  add(event) {
    ...
    return (0, import_lifecycle.toDisposable)((0, import_functional.createSingleCallFunction)(dispose));
  }
  onFirstListenerAdd() {
...
class DynamicListEventMultiplexer {
  constructor(items, onAddItem, onRemoveItem, getEvent) {
    this._store = new import_lifecycle.DisposableStore();
    const multiplexer = this._store.add(new EventMultiplexer());
    const itemListeners = this._store.add(new import_lifecycle.DisposableMap());
...
class Relay {
  constructor() {
    ...
    this.inputEventListener = import_lifecycle.Disposable.None;
    this.emitter = new Emitter({
      onDidAddFirstListener: () => {
        this.listening = true;
