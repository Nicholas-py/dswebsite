...
__export(dom_exports, {
  ...
  addDisposableGenericMouseMoveListener: () => addDisposableGenericMouseMoveListener,
  addDisposableGenericMouseUpListener: () => addDisposableGenericMouseUpListener,
  addDisposableListener: () => addDisposableListener,
  addDisposableThrottledListener: () => addDisposableThrottledListener,
  ...
  clearNode: () => clearNode,
  clearNodeRecursively: () => clearNodeRecursively,
  cloneGlobalStylesheets: () => cloneGlobalStylesheets,
  computeScreenAwareSize: () => computeScreenAwareSize,
  copyAttributes: () => copyAttributes,
  createCSSRule: () => createCSSRule,
  createLinkElement: () => createLinkElement,
  createMetaElement: () => createMetaElement,
  createStyleSheet: () => createStyleSheet,
  createStyleSheet2: () => createStyleSheet2,
  detectFullscreen: () => detectFullscreen,
  disposableWindowInterval: () => disposableWindowInterval,
  ...
  getLargestChildWidth: () => getLargestChildWidth,
  getShadowRoot: () => getShadowRoot,
  getTopLeftOffset: () => getTopLeftOffset,
  getTotalHeight: () => getTotalHeight,
  getTotalScrollWidth: () => getTotalScrollWidth,
  ...
  removeTabIndexAndUpdateFocus: () => removeTabIndexAndUpdateFocus,
  reset: () => reset,
  ...
  setParentFlowTo: () => setParentFlowTo,
  ...
  windowOpenPopup: () => windowOpenPopup,
  windowOpenWithSuccess: () => windowOpenWithSuccess
});
module.exports = __toCommonJS(dom_exports);
var browser = __toESM(require("vs/base/browser/browser"));
var import_canIUse = require("vs/base/browser/canIUse");
...
var event = __toESM(require("vs/base/common/event"));
...
var import_hash = require("vs/base/common/hash");
var import_window = require("vs/base/browser/window");
var import_numbers = require("vs/base/common/numbers");
const {
  registerWindow,
  getWindow,
  getDocument,
  ...
  (0, import_window.ensureCodeWindow)(import_window.mainWindow, 1);
  const mainWindowRegistration = { window: import_window.mainWindow, disposables: new import_lifecycle.DisposableStore() };
  windows.set(import_window.mainWindow.vscodeWindowId, mainWindowRegistration);
  ...
  return {
    ...
    registerWindow(window) {
      ...
      const disposables = new import_lifecycle.DisposableStore();
      const registeredWindow = {
        window,
        disposables: disposables.add(new import_lifecycle.DisposableStore())
      };
      windows.set(window.vscodeWindowId, registeredWindow);
      disposables.add((0, import_lifecycle.toDisposable)(() => {
    ...
    getWindow(e) {
      ...
      return import_window.mainWindow;
    },
    getDocument(e) {
      const candidateNode = e;
...
function _wrapAsStandardMouseEvent(targetWindow, handler) {
  return function(e) {
    return handler(new import_mouseEvent.StandardMouseEvent(targetWindow, e));
  };
}
function _wrapAsStandardKeyboardEvent(handler) {
  return function(e) {
    return handler(new import_keyboardEvent.StandardKeyboardEvent(e));
  };
}
...
function addDisposableGenericMouseDownListener(node, handler, useCapture) {
  return addDisposableListener(node, platform.isIOS && import_canIUse.BrowserFeatures.pointerEvents ? EventType.POINTER_DOWN : EventType.MOUSE_DOWN, handler, useCapture);
}
function addDisposableGenericMouseMoveListener(node, handler, useCapture) {
  return addDisposableListener(node, platform.isIOS && import_canIUse.BrowserFeatures.pointerEvents ? EventType.POINTER_MOVE : EventType.MOUSE_MOVE, handler, useCapture);
}
function addDisposableGenericMouseUpListener(node, handler, useCapture) {
  return addDisposableListener(node, platform.isIOS && import_canIUse.BrowserFeatures.pointerEvents ? EventType.POINTER_UP : EventType.MOUSE_UP, handler, useCapture);
}
function runWhenWindowIdle(targetWindow, callback, timeout) {
  return (0, import_async._runWhenIdle)(targetWindow, callback, timeout);
}
class WindowIdleValue extends import_async.AbstractIdleValue {
...
class AnimationFrameQueueItem {
  ...
  execute() {
    ...
    try {
      ...
      (0, import_errors.onUnexpectedError)(e);
    }
  }
...
class TimeoutThrottledDomListener extends import_lifecycle.Disposable {
  constructor(node, type, handler, eventMerger = DEFAULT_EVENT_MERGER, minimumTimeMs = MINIMUM_TIME_MS) {
    super();
    let lastEvent = null;
    let lastHandlerTime = 0;
    const timeout = this._register(new import_async.TimeoutTimer());
    const invokeHandler = () => {
...
function getActiveDocument() {
  if (getWindowsCount() <= 1) {
    return import_window.mainWindow.document;
  }
  const documents = Array.from(getWindows()).map(({ window }) => window.document);
  return documents.find((doc) => doc.hasFocus()) ?? import_window.mainWindow.document;
}
function getActiveWindow() {
  const document2 = getActiveDocument();
  return document2.defaultView?.window ?? import_window.mainWindow;
}
...
class WrappedStyleElement {
  ...
  setStyle(cssStyle) {
    ...
    if (!this._styleSheet) {
      this._styleSheet = createStyleSheet(import_window.mainWindow.document.head, (s) => s.innerText = cssStyle);
    } else {
...
function createStyleSheet(container = import_window.mainWindow.document.head, beforeAppend, disposableStore) {
  ...
  if (disposableStore) {
    disposableStore.add((0, import_lifecycle.toDisposable)(() => style.remove()));
  }
  if (container === import_window.mainWindow.document.head) {
    const globalStylesheetClones = /* @__PURE__ */ new Set();
    globalStylesheets.set(style, globalStylesheetClones);
    for (const { window: targetWindow, disposables } of getWindows()) {
      if (targetWindow === import_window.mainWindow) {
        continue;
      }
...
function cloneGlobalStyleSheet(globalStylesheet, globalStylesheetClones, targetWindow) {
  ...
  disposables.add((0, import_lifecycle.toDisposable)(() => clone.remove()));
  ...
  disposables.add((0, import_lifecycle.toDisposable)(() => globalStylesheetClones.delete(clone)));
  return disposables;
}
const sharedMutationObserver = new class {
  ...
  observe(target, disposables, options) {
    ...
    const optionsHash = (0, import_hash.hash)(options);
...
function createMetaElement(container = import_window.mainWindow.document.head) {
  return createHeadElement("meta", container);
}
function createLinkElement(container = import_window.mainWindow.document.head) {
  return createHeadElement("link", container);
}
function createHeadElement(tagName, container = import_window.mainWindow.document.head) {
...
class FocusTracker extends import_lifecycle.Disposable {
  constructor(element) {
    super();
...
function windowOpenNoOpener(url) {
  import_window.mainWindow.open(url, "_blank", "noopener");
}
const popupWidth = 780, popupHeight = 640;
function windowOpenPopup(url) {
  const left = Math.floor(import_window.mainWindow.screenLeft + import_window.mainWindow.innerWidth / 2 - popupWidth / 2);
  const top = Math.floor(import_window.mainWindow.screenTop + import_window.mainWindow.innerHeight / 2 - popupHeight / 2);
  import_window.mainWindow.open(
    url,
    "_blank",
...
function windowOpenWithSuccess(url, noOpener = true) {
  const newTab = import_window.mainWindow.open();
  if (newTab) {
    if (noOpener) {
      newTab.opener = null;
    }
...
function animate(targetWindow, fn) {
  ...
  return (0, import_lifecycle.toDisposable)(() => stepDisposable.dispose());
}
...
class ModifierKeyEmitter extends event.Emitter {
  constructor() {
    super();
    this._subscriptions = new import_lifecycle.DisposableStore();
    this._keyStatus = {
      altKey: false,
      shiftKey: false,
      ctrlKey: false,
    ...
    this._subscriptions.add(event.Event.runAndSubscribe(onDidRegisterWindow, ({ window, disposables }) => this.registerListeners(window, disposables), { window: import_window.mainWindow, disposables: this._subscriptions }));
  }
  registerListeners(window, disposables) {
    disposables.add(addDisposableListener(window, "keydown", (e) => {
      ...
      const event2 = new import_keyboardEvent.StandardKeyboardEvent(e);
      if (event2.keyCode === import_keyCodes.KeyCode.Alt && e.repeat) {
        return;
      }
      ...
      } else if (event2.keyCode !== import_keyCodes.KeyCode.Alt) {
...
class DragAndDropObserver extends import_lifecycle.Disposable {
...
class SafeTriangle {
  ...
  contains(x, y) {
    ...
    for (let i = 0; i < 4; i++) {
      if ((0, import_numbers.isPointWithinTriangle)(x, y, originX, originY, triangles[2 * i], triangles[2 * i + 1], triangles[2 * i + 2], triangles[2 * i + 3])) {
        return true;
      }
    }
    return false;
