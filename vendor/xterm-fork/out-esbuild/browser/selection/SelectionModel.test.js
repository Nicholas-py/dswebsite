...
var import_SelectionModel = require("./SelectionModel");
...
describe("SelectionModel", () => {
  let model;
  beforeEach(() => {
    const bufferService = new import_TestUtils.MockBufferService(80, 2);
    model = new import_SelectionModel.SelectionModel(bufferService);
  });
  describe("clearSelection", () => {
    it("should clear the final selection", () => {
      model.selectionStart = [0, 0];
      model.selectionEnd = [10, 2];
      import_chai.assert.deepEqual(model.finalSelectionStart, [0, 0]);
      import_chai.assert.deepEqual(model.finalSelectionEnd, [10, 2]);
      model.clearSelection();
      import_chai.assert.deepEqual(model.finalSelectionStart, void 0);
      import_chai.assert.deepEqual(model.finalSelectionEnd, void 0);
    });
  });
  describe("areSelectionValuesReversed", () => {
    it("should return true when the selection end is before selection start", () => {
      ...
      import_chai.assert.equal(model.areSelectionValuesReversed(), true);
    ...
    it("should return false when the selection end is after selection start", () => {
      ...
      import_chai.assert.equal(model.areSelectionValuesReversed(), false);
      model.selectionStart = [0, 0];
      model.selectionEnd = [10, 2];
      import_chai.assert.equal(model.areSelectionValuesReversed(), false);
    });
  ...
  describe("onTrim", () => {
    it("should trim a portion of the selection when a part of it is trimmed", () => {
      ...
      model.handleTrim(1);
      import_chai.assert.deepEqual(model.finalSelectionStart, [0, 0]);
      import_chai.assert.deepEqual(model.finalSelectionEnd, [10, 1]);
      model.handleTrim(1);
      import_chai.assert.deepEqual(model.finalSelectionStart, [0, 0]);
      import_chai.assert.deepEqual(model.finalSelectionEnd, [10, 0]);
    });
  ...
  describe("finalSelectionStart", () => {
    ...
    it("should return selection start if there is no selection end", () => {
      ...
      import_chai.assert.deepEqual(model.finalSelectionStart, [2, 2]);
    });
    it("should return selection end if values are reversed", () => {
      ...
      import_chai.assert.deepEqual(model.finalSelectionStart, [1, 2]);
    });
  });
  describe("finalSelectionEnd", () => {
    it("should return the end of the buffer if select all is active", () => {
      model.isSelectAllActive = true;
      import_chai.assert.deepEqual(model.finalSelectionEnd, [80, 1]);
    });
    it("should return null if there is no selection start", () => {
      import_chai.assert.equal(model.finalSelectionEnd, void 0);
      model.selectionEnd = [1, 2];
      import_chai.assert.equal(model.finalSelectionEnd, void 0);
    });
    it("should return selection start + length if there is no selection end", () => {
      ...
      import_chai.assert.deepEqual(model.finalSelectionEnd, [4, 2]);
    });
    ...
    it("should return the end on a different row when start + length overflows onto a following row", () => {
      ...
      import_chai.assert.deepEqual(model.finalSelectionEnd, [2, 3]);
    });
    ...
    it("should return selection end if selection end is after selection start + length", () => {
      ...
      import_chai.assert.deepEqual(model.finalSelectionEnd, [5, 2]);
    });
    it("should not include a trailing EOL when the selection ends at the end of a line", () => {
      model.selectionStart = [0, 0];
      model.selectionStartLength = 80;
      import_chai.assert.deepEqual(model.finalSelectionEnd, [80, 0]);
      model.selectionStart = [0, 0];
