...
test_1.test.describe('Mouse Tracking Tests', () => {
    ...
    test_1.test.describe('DECSET 9 (X10)', async () => {
        (0, test_1.test)('default encoding', async () => {
            ...
            await (0, TestUtils_1.pollFor)(ctx.page, () => getReports(encoding), [{ col: 1, row: 1, state: { action: 'press', button: 'left', modifier: { control: false, shift: false, meta: false } } }]);
            await mouseUp('left');
            await (0, TestUtils_1.pollFor)(ctx.page, () => getReports(encoding), []);
            await mouseMove(50, 10);
            await (0, TestUtils_1.pollFor)(ctx.page, () => getReports(encoding), []);
            await mouseDown('left');
            await mouseUp('left');
            await (0, TestUtils_1.pollFor)(ctx.page, () => getReports(encoding), [{ col: 51, row: 11, state: { action: 'press', button: 'left', modifier: { control: false, shift: false, meta: false } } }]);
            await mouseMove(223 - 1, rows - 1);
            await mouseDown('left');
            await mouseUp('left');
            await (0, TestUtils_1.pollFor)(ctx.page, () => getReports(encoding), [{ col: 223, row: rows, state: { action: 'press', button: 'left', modifier: { control: false, shift: false, meta: false } } }]);
            ...
            await mouseUp('left');
            await (0, TestUtils_1.pollFor)(ctx.page, () => getReports(encoding), [{ col: 44, row: 25, state: { action: 'press', button: 'left', modifier: { control: false, shift: false, meta: false } } }]);
            await mouseMove(43, 24);
            await getReports(encoding);
            await mouseDown('right');
            await mouseMove(44, 24);
            await mouseUp('right');
            await (0, TestUtils_1.pollFor)(ctx.page, () => getReports(encoding), [{ col: 44, row: 25, state: { action: 'press', button: 'right', modifier: { control: false, shift: false, meta: false } } }]);
            await mouseMove(43, 24);
            await getReports(encoding);
        ...
        (0, test_1.test)('SGR encoding', async () => {
            if (ctx.browser.browserType().name() === 'webkit') {
                test_1.test.skip();
                return;
            }
            ...
            await mouseDown('left');
            await mouseUp('left');
            await (0, TestUtils_1.pollFor)(ctx.page, () => getReports(encoding), [{ col: cols, row: rows, state: { action: 'press', button: 'left', modifier: { control: false, shift: false, meta: false } } }]);
            await mouseMove(43, 24);
            await getReports(encoding);
            await mouseDown('left');
            await mouseMove(44, 24);
